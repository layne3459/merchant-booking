...
export function formatBusinessHours(hours?: Record<string, string[]> | null) {
  if (!hours || typeof hours !== 'object') return '周一至周日 09:00 - 21:00';
  const sat = hours.sat?.join('-');
  const sun = hours.sun?.join('-');
  const weekday = hours.mon?.join('-') || '09:00-21:00';
  if (sat && sun && sat !== weekday) {
    return `工作日 ${weekday} · 周末 ${sat}`;
  }
  return `每日 ${weekday}`;
}
